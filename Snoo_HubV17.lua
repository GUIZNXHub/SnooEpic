...
local SpamTPV1ButtonSmall = Instance.new("TextButton")
...
local SpamTPV2ButtonSmall = Instance.new("TextButton")
...
local function ShowBestNotice(Name)
...
local BestLineEnabled = LoadedConfig.BestLineEnabled == true
...
ESPPlots = workspace:FindFirstChild("Plots")
if ESPPlots then
	table.insert(ESPConnections, ESPPlots.DescendantAdded:Connect(function()
		ESPCacheDirty = true
		if UltraESPEnabled then
			local Target = ESPFindBestTarget(true)
			if Target then CurrentBestTarget = Target; ESPCurrentTarget = Target end
		end
	end))
	table.insert(ESPConnections, ESPPlots.DescendantRemoving:Connect(function()
		ESPCacheDirty = true
		if UltraESPEnabled or BestLineEnabled then
			local Target = ESPFindBestTarget(true)
			CurrentBestTarget = Target; ESPCurrentTarget = Target
		end
	end))
end
...
print("SnooHubV17 carregado corretamente.")
